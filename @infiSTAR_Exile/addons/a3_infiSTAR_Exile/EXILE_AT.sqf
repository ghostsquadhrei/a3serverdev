...
infiSTAR_MAIN_CODE = "
	...
	fnc_draw3dhandlerAI = ""
		...
		{
			_maxD = 800;
			ALLVEHICLES_OBJECTS_800m = cameraOn nearEntities ['Allvehicles',_maxD];
			{
				if(!isNull _x)then
				{
					if(alive _x)then
					{
						...
						if(_PUIDX == '')then
						{
							...
							if((_x isKindOf 'Man')&&!(_x isKindOf 'Animal_Base_F'))then
							{
								...
								if(isNull _grp)then
								{
									_txt = format['%1 %2m (%3HP)',_name,round _distance,round((damage _x - 1) * -100)];
								}
								else
								{
									_txt = format['%1 %2m (%3HP) %4',_name,round _distance,round((damage _x - 1) * -100),allGroups find _grp];
								};
								_pos = ASLToATL eyepos _x;
								if(surfaceIsWater _pos)then{_pos = eyepos _x;};
								drawIcon3D['',[1,1,1,0.7],_pos,.1,.1,0,_txt,1,.032];
							};
						};
					};
				};
			} forEach ALLVEHICLES_OBJECTS_800m;
		};
	"";
						...
						if(_PUIDX == '')then
						{
							...
							if(mapiconsshowai)then
							{
								if((_x isKindOf 'Man') && !(_x isKindOf 'Animal_Base_F'))then
								{
									...
									if(isNull _grp)then
									{
										_txt = format['%1 (%2HP)',_txt,round((damage _x - 1) * -100)];
									}
									else
									{
										_txt = format['%1 (%2HP) %3',_txt,round((damage _x - 1) * -100),allGroups find _grp];
									};
									_icon = getText(configFile >> 'CfgVehicles' >> _type >> 'icon');
