...
_allRepaired = true;
{
        ...
        if ((vehicle player != _vehicle) || (!local _vehicle) || ([0,0,0] distance (velocity _vehicle) > 1)) exitWith {
                _allRepaired = false;
				["Whoops", format ["Reparation du %1 stoppé", _name]] call ExileClient_gui_notification_event_addNotification;
        };
        ...
        if (_damage > 0) then {
                if (_repairTime > 0) then {
                        ...
                        //titleText [format["Réparation en cours %1 ...Patientez...", _partName], "PLAIN DOWN", _repairTime];
						//sleep _repairTime;
                };
...
if (_allRepaired) then {
		...
		["Success", format ["Le %1 est réparé", _name]] call ExileClient_gui_notification_event_addNotification;
};
